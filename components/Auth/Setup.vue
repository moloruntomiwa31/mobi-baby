		...
		<form class="bg-white w-4/5 max-w-5xl px-8 py-6 rounded-xl shadow-lg">
			...
			<div class="flex items-center justify-center gap-2 mb-8">
				<template v-for="(option, index) in visibleOptions" :key="option.id">
					...
					>
						<span
							class="size-8 rounded-full flex items-center justify-center bg-current"
						>
			...
			<!-- Main Content -->
			<div
				class="flex flex-col mdlg:flex-row gap-8 items-center justify-center w-full"
			>
				<!-- Upload Section -->
				<div
					v-if="tab === 'profile'"
					class="bg-primaryGray w-40 h-40 mdlg:w-80 mdlg:h-80 rounded-full flex flex-col items-center justify-center gap-3 p-4 shadow-md hover:bg-opacity-90 transition"
				>
					...
					<BaseText
						size="xs"
						:content="
							type === 'parent' ? 'Upload baby photo here' : 'Upload your photo'
						"
				...
				<div class="w-full mdlg:w-3/5 flex flex-col gap-6">
					<template v-if="tab === 'profile' && type === 'parent'">
						<div class="grid grid-cols-2 gap-6">
							<div class="flex flex-col gap-2">
								...
								<Input
									id="parent"
									type="text"
									placeholder="Aminat Shekirat"
									class="bg-gray p-4 rounded-md w-full"
								/>
							</div>
							<div class="flex flex-col gap-2">
						...
						<div class="flex flex-col gap-2">
							...
							<Input
								id="email"
								type="email"
								placeholder="aminat123@gmail.com"
								class="bg-gray p-4 rounded-md w-full"
							/>
						</div>
						<div class="flex flex-col gap-2">
					...
					<template v-if="tab === 'type' && type === 'parent'">
						<div class="grid md:grid-cols-2 gap-6 md:gap-2">
							<div class="flex flex-col gap-2">
								<Label for="vaccination"
									>Has your baby had their first vaccination?</Label
								>
								<BaseSelect
									id="vaccination"
							...
							<div class="flex flex-col gap-2">
								<Label for="vaccination"
									>What vaccination has your baby received?</Label
								>
								<BaseSelect
									id="vaccination"
									multiple
						...
						<div class="grid md:grid-cols-2 gap-6 md:gap-2 w-full">
							<div class="flex flex-col gap-2">
								<Label for="feeding"
									>How's your baby currently being fed?</Label
								>
								<BaseSelect
									id="feeding"
									:options="[
					...
					<template v-if="tab === 'type' && type === 'medical-practitioner'">
						<div class="flex flex-col gap-2">
							...
							<Input
								id="clinic"
								type="text"
								placeholder="General Hospital, Kano"
								class="bg-gray p-4 rounded-md w-full"
							/>
						</div>
						<div class="flex flex-col gap-2">
			...
			<div class="w-full flex justify-between mt-6">
				...
				<Button v-else :disabled="isLastStep" @click.prevent="nextStep">
					Next
				</Button>
			</div>
		</form>
...
<script setup lang="ts">
	...
	const accountSetupOptions = computed(() => [
		...
		{
			name: type.value === "parent" ? "Details" : "Experience",
			id: "type",
			hide: false,
			done: false,
		},
	...
	const visibleOptions = computed(() =>
		accountSetupOptions.value.filter((o) => !o.hide)
	);
	const currentIndex = computed(() =>
		visibleOptions.value.findIndex((t) => t.id === tab.value)
	);

	const isFirstStep = computed(() => currentIndex.value === 0);
	const isLastStep = computed(
		() => currentIndex.value === visibleOptions.value.length - 1
	);
