			...
			>
				...
				>
					...
					<div class="flex flex-col items-start">
						...
						<BaseText as="span" size="xs" clamp>{{
							task.description
						}}</BaseText>
					</div>
				</button>
		...
		<template v-for="m in materials" :key="m.title">
			...
			>
				<div class="flex items-center justify-between w-full">
					...
					<NuxtLink to="/feeds" v-if="isMobile" class="text-primaryAqua"
						>See all</NuxtLink
					>
				</div>
				<div
					class="flex mdlg:flex-col items-center gap-4 overflow-x-auto flex-nowrap whitespace-nowrap no-scrollbar w-full"
				>
					<MaterialCard
						v-for="data in m.list"
						:key="data.description"
						:wrapped="isMobile ? true : false"
						:m="data"
					/>
				</div>
				<NuxtLink to="/feeds" v-if="!isMobile" class="text-primaryAqua"
